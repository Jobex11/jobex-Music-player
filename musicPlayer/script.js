...
//actually code for >> let musicIndex = math.floor(Math.random() * allMusic.length + 1) not 1;
let musicIndex = 1;
...
// Next Music function
function nextMusic() {
  musicIndex++; //increment of musicindex
  //if music index is greater than array length to enable music return to 1
...
// update progress widht according to music current time
mainAudio.addEventListener("timeupdate", (e) => {
  const currentTime = e.target.currentTime; // get playing song current time
  const duration = e.target.duration; //get playing total duration
...
repeatBtn.addEventListener("click", () => {
  let getText = repeatBtn.innerText; // geting this tag innerText
  switch (getText) {
    case "repeat":
      repeatBtn.innerText = "repeat_one";
      //I CHANGE SONG REPEAT ..song looped
...
mainAudio.addEventListener("ended", () => {
  ...
  switch (getText) {
    ...
    case "repeat_one":
      ...
      loadMusic(musicIndex); //calling load music function with argument, in the argument there is an index of the current song
    ...
    case "shuffle":
      ...
      } while (musicIndex == randIndex); // this loop run untill the next random
...
//let create li tags accordin to array lenght for list
...
function playingSong() {
  for (let j = 0; j < allLiTags.length; j++) {
    ...
    //let remove playing class from all other li except the last one which is clicked
